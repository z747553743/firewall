...
int delete_rule_chain_node_by_info(struct rule_chain ** head,
    unsigned int s_addr, unsigned int s_mask, unsigned short s_port,
    unsigned int d_addr, unsigned int d_mask, unsigned short d_port,
    unsigned char proto, unsigned char policy)
{
    ...
    while(tmp_node != NULL){
        ...
        if(s_addr == tmp_node->s_addr && s_mask == tmp_node->s_mask && s_port == tmp_node->s_port &&
	        d_addr == tmp_node->d_addr && d_mask == tmp_node->d_mask && d_port == tmp_node->d_port &&
	        proto == tmp_node->proto && policy == tmp_node->policy){
        	delete_rule_chain_node(head, tmp_node);
        	return 0;
        }
        tmp_node = next_node;
    }
